...
let editMember = async (values) => {
  ...
  for (let d of data) {
    let _sql = `INSERT INTO edu_member (name, parents, age, tel_phone, customer_type) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE name=VALUES(name),parents=VALUES(parents),age=VALUES(age),tel_phone=VALUES(tel_phone),customer_type=VALUES(customer_type)`
    let arrStr = []
    arrStr.push( d.name, d.parents, d.age, d.tel_phone, d.customer_type, )
    if(d.id){
      arrStr.push(d.id)
      _sql = `INSERT INTO edu_member ( name, parents, age, tel_phone, customer_type , id) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE name=VALUES(name),parents=VALUES(parents),age=VALUES(age),tel_phone=VALUES(tel_phone),customer_type=VALUES(customer_type)`
    }
...
let downloadMemberExcel = async (values, ctx) => {
  ...
  ctx.set('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
  ctx.set('Character-Encoding', 'utf-8')
  let fileName = `会员管理${moment().format("YYYY-MM-DD hhmmss")}`
  ctx.set("Content-Disposition", `attachments; filename=${encodeURIComponent(fileName)}.xlsx`)
  return wb.writeToBuffer()
}
// 查询会员详情
let getMemberDetail = async (values) => {
  let {data} = values
  let _sqlChilds = `SELECT t3.* from edu_member t1 left join edu_parents_childs t2 on t1.id = t2.member_id left join edu_childs t3 on t3.id = t2.childs_id 
