...
let editChild = async (values) => {
  ...
  for (let d of data) {
    ...
    if(d.class_id && +d.member_status === 1) {
     ...
     //查询旧的课程id和费用
     _sqlE = `SELECT t1.class_id, t2.final_fee from edu_parents_child t1 join edu_child t2  where t1.child_id = t2.id and t2.id = ${childId}`
     let childResult = await query(_sqlE)
     // 如果旧的课程id 和 新选择的课程id一样 就插入旧的费用
     if(+childResult[0].class_id === +d.class_id){
      finalClass_fee = childResult[0].final_fee
     } else {
      finalClass_fee = classResult[0].class_fee
     }
     arrStrC = [ finalClass_fee, childId ]
...
let editSalary = async (values) => {
  ...
  for (let d of data) {
    ...
    _sqlA = `SELECT id FROM edu_staff_salary where deleted = 0 and staff_id = ${d.staff_id} ORDER BY id DESC LIMIT 1`
...
let deleteSalary = async(values) => {
  ...
  _sqlA = `SELECT id,salary,salary_time FROM edu_staff_salary where deleted = 0 and staff_id = ${staff_id} ORDER BY id DESC LIMIT 1`
  let resultNew = await query(_sqlA)
