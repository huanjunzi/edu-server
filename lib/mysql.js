...
let findMember = (values) => {
  ...
  on t1.id = t2.member_id where ${filterSql}${paramsSql}${pageSql}${searchSql}${timeSql}${orderSql}`
  return query(_sql)
}
...
let downloadMemberExcel = async (values, ctx) => {
  ...
  for (let i in data) {
    ...
    ws.cell(index, 6).string(row.child_count + '')
...
let findChild = (values) => {
  ...
  left join edu_member member on member.id = t1.member_id where ${filterSql}${paramsSql}${pageSql}${searchSql}${timeSql}${orderSql}`
  return query(_sql)
}
// 下载儿童信息的Excel
...
let editChild = async (values) => {
  ...
  let childResult = ""
  for (let d of data) {
    _sqlD = `SELECT child_count from edu_member where id = ${d.member_id}`
    ...
    if(d.id) {
      ...
      childResult = await query(_sqlA, arrStrA)
      if(childResult && d.class_id) {
        const _sqlE = `SELECT * from edu_class where id = ${d.class_id}`
        let class_data = await query(_sqlE)
        let fee = [childResult.insertId,class_data[0].class_fee]
        _sqlF = `INSERT INTO edu_child (id, final_fee) VALUES (?,?) ON DUPLICATE KEY UPDATE final_fee=VALUES(final_fee)`
        await query(_sqlF, fee)
      }
    }
    // 如果是insert
