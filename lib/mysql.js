...
let findClasses = (values) => {
  ...
  let _sql = `SELECT * from edu_class where ${filterSql}${searchSql}${paramsSql}${timeSql}${pageSql}`
...
let downloadExcel = async (values, ctx) => {
  ...
  ws.row(3).setHeight(60)
  ws.row(4).setHeight(60)


  ctx.set('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
  ctx.set('Character-Encoding', 'utf-8')
  let fileName = `课程项目${moment().format("YYYY-MM-DD_hh:mm:ss")}`
