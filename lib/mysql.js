...
let editChild = async (values) => {
  ...
  for (let d of data) {
    ...
    !_.isEmpty(d.birthday) ? moment(d.birthday).format("YYYY-MM-DD") : d.birthday = null
...
let editStaff = async (values) => {
  ...
  data = JSON.parse(data)
  for (let d of data) {
    !_.isEmpty(d.dimission_time) ? moment(d.dimission_time).format("YYYY-MM-DD") : d.dimission_time = null
